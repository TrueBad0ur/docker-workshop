...
func NewUploadController(db *gorm.DB, minioClient *minio.Client, redisClient *redis.Client, ctx context.Context) *UploadController {
...
func (uc *UploadController) HandleBulkDownload() error {
	for i, url := range imageUrls {
		...
		_, err = uc.MinioClient.PutObject(uc.Ctx, "premium-images", imageID+".jpg", reader, reader.Size(), minio.PutObjectOptions{
			ContentType: "application/octet-stream",
		})
		if err != nil {
			return err
